...
for idx = 1 : 3  
    ...
    for dir_idx = 1 : params.dir_skip : length(im_dirs)
        ...
        imsubdir = dir_bo(fullfile(data_path, 'rgbjpg'));
        impath = cell(1, length(1 : params.file_skip : length(imsubdir)));
        file_idx = 1;
        for img_idx = 1 : params.file_skip : length(imsubdir)
            impath{file_idx} = fullfile(data_path, 'rgbjpg', imsubdir(img_idx).name);
            file_idx = file_idx +  1;
        end
        savedir = fullfile(rt_dir, 'kdes', 'color_rgb', im_class{double(idx > 1) + 1});
...
for idx = 1 : length(kdes_path)
    ...
    rgbdfea = cksvd_emk_batch(fea_sparams, basis_params, emk_params);
    rgbdfea = single(rgbdfea);
    %}
    save -v7.3 rgbdfea_rgb_nrgkdes rgbdwords;
end
%rmdir('~/Projects/WNP-Preprocess/Dataset/kdes/color_rgb', 's');
