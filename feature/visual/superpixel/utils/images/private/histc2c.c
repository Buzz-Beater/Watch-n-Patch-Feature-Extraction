...
*******************************************************************************/
int findBin( double x, double *edges, mwSize nBins ) {
  mwSize k = nBins; /* NOBIN */
  int k0 = 0; mwSize k1 = nBins;
...
mwSize *sub2ind_init( const mwSize*siz, const int nd ) {
  mwSize i, *subMul;
  subMul = (mwSize*) mxCalloc( nd, sizeof(mwSize));
...
void histcND( double* h, double* A, double* wtMask, int n, int nd, double**edges, mwSize* nBins ) {
  int i, j, k, inbounds; int *sub, ind;
  mwSize *subMul;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  int i, n, nd;
  mwSize *nBins; double *A, *wtMask, **edges, *h;
  ...
  nBins = (mwSize*) mxMalloc( nd * sizeof(mwSize) );
