...
%
function [action2aligned] = getAlignedSkeleton(action2anchor, action2skeleton)
	...
	for action_idx = 1 : action_cnt
		...
		for skeleton_idx = 1 : size(action2skeleton{action_idx}, 2)
			skeleton = action2skeleton{action_idx}{skeleton_idx};
			...
			action2aligned{action_idx}{skeleton_idx} = aligned_joints';
		end
	end
end
